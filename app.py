    ...
    if "Tasks" in sheets:
        ...
        def save_plot_as_image(fig):
            try:
                ...
                print("⚠️ Could not generate chart image:", e)
                placeholder = BytesIO()
                c = canvas.Canvas(placeholder, pagesize=(600, 400))
                c.setFont("Helvetica-Bold", 14)
                c.drawCentredString(300, 200, "Chart export unavailable (Kaleido missing)")
                c.save()
                placeholder.seek(0)
                return ImageReader(placeholder)

...
with tabs[0]:
    ...
    if not df_install.empty:
        ...
        if contractor_col and status_col:
            ...
            def make_contractor_gauge(completed, total, title, dial_color="#007acc"):
                ...
                fig = go.Figure(
                    go.Indicator(
                        mode="gauge+number",
                        value=pct,
                        # number font kept but will be visually minimized (we render our own responsive percentage below)
                ...
                # keep modest height; the responsive percentage below will visually center itself
            ...
            # ---- ADD: Top Tiles with Deezlo / Isandiso / Nimba (if present) ----
            # This creates a top row of tiles above the gauges showing the three contractors, matching your screenshot.
            try:
                top_order = ["deezlo", "isandiso", "nimba"]
                # build mapping of contractor -> record for quick lookup (case-insensitive)
                rec_map = {str(r.get(contractor_col, "")).strip().lower(): r for r in records}
                top_tiles = []
                for name in top_order:
                    if name in rec_map:
                        top_tiles.append(rec_map[name])
                    else:
                        top_tiles.append(None)
                # Render the three top tiles
                cols_top = st.columns(3)
                for k, tile in enumerate(top_tiles):
                    with cols_top[k]:
                        if tile is not None:
                            cname = tile.get(contractor_col, "")
                            # styled top tile label
                            st.markdown(f"<div class='metric-card' style='padding:0.6rem 0.8rem;'><div style='text-align:center; font-weight:700; color:#e67300; letter-spacing:1px;'>{str(cname).upper()}</div></div>", unsafe_allow_html=True)
                        else:
                            # empty placeholder to keep spacing consistent
                            st.markdown(f"<div style='height:34px;'></div>", unsafe_allow_html=True)
            except Exception:
                # if anything goes wrong rendering top tiles, ignore and continue (preserve original behavior)
                pass
            ...
            for i in range(0, len(records), 3):
                ...
                for j, rec in enumerate(records[i : i + 3]):
                    ...
                    with cols[j]:
                        ...
                        # ---- Move Contractor Name ABOVE the gauge (ensures name is top tile label for every gauge) ----
                        # We keep the original gauge title blank and show the name here so names appear consistently above.
                        try:
                            contractor_name_display = str(rec.get(contractor_col, "")).upper()
                        except Exception:
                            contractor_name_display = str(rec.get(contractor_col, ""))
                        ...
                        # Render the gauge (title suppressed inside the gauge so it's visually clean)
                        st.plotly_chart(make_contractor_gauge(completed, total, str(rec.get(contractor_col, "")), dial_color=color), use_container_width=True)

                        # ---- Centered responsive percentage (will scale with window using vw) ----
                        # font-size uses calc to combine base px with viewport width so it remains centered and scales.
                        st.markdown(
                            f"<div style='text-align:center; font-size:calc(14px + 1.4vw); font-weight:600; color:{color}; margin-top:-12px;'>{pct:.1f}%</div>",
                            unsafe_allow_html=True,
                        )

                        # numeric label under the dial (kept from original)

...
sheet_choice = st.sidebar.selectbox("Sheet to view", list(sheets.keys()),
...
show_notstarted = st.sidebar.checkbox("Show Not Started", True)
show_inprogress = st.sidebar.checkbox("Show In Progress", True)
...
if not df_main.empty:
    ...
    progress_map = []
    if show_notstarted: progress_map.append("Not Started")
    if show_inprogress: progress_map.append("In Progress")
    if show_completed: progress_map.append("Completed")
...
if "Tasks" in sheets:
    ...
    def create_gauge(value, total, title, colors):
        ...
        fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=pct,
            number={"suffix":"%", "font":{"size":40,"color":"darkblue"}},
            gauge={
                "axis":{"range":[0,100]},
        ...
        fig.add_annotation(text=f"<b>{title}</b>", x=0.5, y=1.25, showarrow=False,
                           font=dict(size=18,color="darkblue"), xanchor="center")
        fig.add_annotation(text=f"{value} of {total} tasks", x=0.5, y=-0.25, showarrow=False,

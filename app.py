...
if df_main.empty:
    ...
    if search_task:
        df_main = df_main[
            df_main[df_main.columns[0]]
            .astype(str)
            .str.contains(search_task, case=False, na=False)
        ]
    ...
    if "Tasks" in sheets:
        ...
        inprogress = (
            tasks["Progress"].str.lower().eq("in progress").sum()
            if "Progress" in tasks.columns
            else 0
        )
        notstarted = (
            tasks["Progress"].str.lower().eq("not started").sum()
            if "Progress" in tasks.columns
            else 0
        )
        overdue = (
            (
                ...
                & (~tasks["Progress"].str.lower().eq("completed"))
            ).sum()
    ...
    if sheet_choice == "Tasks" or "Tasks" in sheets:
        ...
        # Priority distribution (show % inside slices)
        ...
        if "Due date" in tasks.columns and "Progress" in tasks.columns:
            ...
            st.dataframe(
                overdue_df.sort_values("Due date")
                .reset_index(drop=True)
                .loc[:, ["Task Name", "Bucket Name", "Progress", "Due date", "Priority"]]
                .head(200)
            )
        ...
        if "Completed Checklist Items" in tasks.columns:
            ...
            if tasks["check_pct"].notna().any():
                ...
                st.dataframe(
                    tasks[["Task Name", "Completed Checklist Items", "check_pct"]]
                    .sort_values("check_pct", ascending=False)
                    .head(200)
                )
